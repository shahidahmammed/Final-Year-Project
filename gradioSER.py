...
# inputs
heading = "Speech Emotion Recognition" # heading in the webpage
userAudio = gr.inputs.Audio(source="upload", type="file", label="Upload to recognize the emotion!") # upload an audio
userAudioMIC = gr.inputs.Audio(source="microphone", type="file", label="Start Recording to recognize emotion!") # record  an audio
...
predictionMIC = gr.outputs.Label(label="Emotion Recognized is") # output of recorded audio
...
recordAudio = gr.Interface(fn=SER, inputs=[userAudioMIC, imageAccuracy, numberEmotions], outputs=[predictionMIC, predictionACC], title=heading, description=description, allow_flagging="never")  # record interface
demo = gr.TabbedInterface([uploadAudio, recordAudio], ["Upload an audio", "Record an audio"]) # tabbed both interfaces
demo.launch() # launches interface on web-browser